...
- ✅ **Scheduler reescanea todo el disco cada tick** — `harness-core/scheduler/tick.rs` — **RESUELTO 2026-06-09 Wave 3**: scheduler indexado, snapshot en memoria por thread, write-through en operaciones, scheduler consume snapshot() sin TOML reads en ticks estables. — **ALTA / M**
...
- ✅ **`read_output` lee el `output.log` entero (hasta 50 MiB) en el handler SSE async** — `harness-server/routes/events.rs` — **RESUELTO 2026-06-09 Wave 3**: read_active_chunk() streaming incremental 256 KiB, spawn_blocking para SSE/events, paginación sin bufferizar 50 MiB. — **ALTA / M**
...
- ✅ **`seq = read_events().len()`** relee y deserializa todo el log por append (T7) — **RESUELTO 2026-06-09 Wave 3**: seq asignado atómicamente por thread bajo write_lock en append_event; contador inicializado del mayor seq en disco. Cierra race condition. — **MEDIA / S**
...
- **Lock poisoning (T8)** — ✅ Resuelto 2026-06-09 (Wave 2): recuperación con `into_inner()` + warn en governor/store/manager/session. Anterior: `harness-server/approvals.rs:80,93,105,122`, `harness-policy/engine.rs:87,97,141`. — **MEDIA / S** — **CERRADO**
