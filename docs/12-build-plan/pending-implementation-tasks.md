...
14. **Task 13: Separar `task.create` y `task.propose`** — **ejecutada (scope local, 2026-06-03)** vía dogfooding (codex ejecutor + review Sonnet). Entregado: deny-by-default por `--role` confiable en la capa MCP (`harness-mcp-server` recibe nuevo flag `--role`; `can_create_tasks` deniega `task_create` a workers → usan `task_propose`); modelo `TaskProposal` append-only (`propose`/`list_proposals`/`promote`) + endpoints REST `/task-proposals(+/promote)` + evento `task.proposal.*`; `discovered_by_role` proviene del `--role` confiable (no spoofable). 127 tests verdes. **Pendiente diferido a Task 14:** el gate REST de `promote` por rol es no-autoritativo (la API usa token compartido sin identidad por-caller); aceptado para harness local single-user, se cierra con el capability middleware. La auditoría de `harness-policy` concluyó que el motor es por tool+args sin actor → el deny por rol vive en el dispatcher MCP, no en `harness-policy`.
15. **Task 14: Capability policy middleware mínimo** — enforcement real de roles/tools/resources. **Incluye** la identidad por-caller que falta en REST (el gate de `promote`/mutating endpoints de Task 13 depende de esto; hoy spoofable por body bajo token compartido).
