<script lang="ts">
  ...
  import { createAgentSession, lookupCurrentRepo } from '$lib/features/agents/handlers';
  import type { NewSessionFormData } from '$lib/features/agents/entities';
  ...
  async function lookupRepo() {
    ...
    repoLookup = 'loading';
    try {
      const res = await lookupCurrentRepo(requestedCwd);
  ...
  async function createSession() {
    ...
    try {
      ...
      const res = await createAgentSession({ threadId, form, repoReport });
      open = false;
      reset();
      if (onCreated) {
        onCreated({ threadId: res.threadId, sessionId: res.sessionId });
      } else {
        await goto(`/threads/${res.threadId}/sessions/${res.sessionId}`);
      }
    } catch (err) {
    ...
    <form class="mt-4 flex flex-col gap-4" onsubmit={submit}>
      ...
      <DialogFooter>
        ...
        <button
          ...
          class="inline-flex h-9 items-center justify-center gap-2 whitespace-nowrap rounded-md bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground shadow-[var(--shadow-primary)] transition-[background-color,color,box-shadow,transform] hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-ring)] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]"
        >
          {#if submitting}
